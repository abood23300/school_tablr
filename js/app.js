...
async function generateTimetable() {
  ...
  // خوارزمية التوزيع الفعلية تعمل على خادم Supabase Edge Function
  // (وليس في هذا الملف) حتى لا تكون مرئية لمن يفتح "عرض المصدر" على الموقع.
  const genBtn = document.getElementById('generateBtn');
  const originalBtnText = genBtn?.textContent;
  if (genBtn) { genBtn.disabled = true; genBtn.textContent = 'جارٍ التوليد...'; }

  try {
    const { data, error } = await supabaseClient.functions.invoke('generate-timetable', {
      body: { school: st.school, classes, subjects, teachers },
    });

    if (error) {
      alert('تعذّر توليد الجدول: ' + (error.message || 'خطأ غير متوقع من الخادم'));
      return [];
    }
    if (data?.error) {
      alert(data.error);
    ...
    alert('تعذّر الاتصال بخادم التوليد: ' + (err?.message || err));
    return [];
  } finally {
    if (genBtn) { genBtn.disabled = false; genBtn.textContent = originalBtnText || 'توليد الجدول'; }
  }
}
